...
// GET all specials for a shop
router.get('/', async (req, res) => {
  try {
    const { shopId } = req.query;
    if (!shopId) return res.status(400).json({ error: 'shopId required' });
    const now = new Date().toISOString();
    // Filter out expired specials based on expiresAt time (HH:MM format)
    const now = new Date();
...
// POST create special (whitelist fields)
router.post('/', async (req, res) => {
  try {
    const b = req.body;
    if (!b.shopId || !b.name || !b.price) return res.status(400).json({ error: 'shopId, name, price required' });
    const data = {
      shopId:        b.shopId,
