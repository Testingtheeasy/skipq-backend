...
router.post('/register', async (req, res) => {
  try {
    ...
    // Try with password field, fallback without if column doesn't exist yet
    let customer;
    try {
      customer = await db(p => p.customer.create({
        data: { phone, name, password: hashPassword(password), points: 0 }
      }));
    } catch (e) {
      if (e.message && e.message.includes('password')) {
        // Password column not added yet - create without it
        customer = await db(p => p.customer.create({
          data: { phone, name, points: 0 }
        }));
      } else { throw e; }
    }
