...
router.post('/', async (req, res) => {
  try {
    ...
    // Same sequential counter for both regular and pre-orders (one merged,
    // chronological list is easier for staff than tracking two separate
    // sequences) - pre-orders just get a "P" tag so they're visually
    // distinguishable at the counter/kitchen screen.
    const token  = prefix + '-' + (isPreOrder ? 'P' : '') + String(count + 1).padStart(3, '0');
